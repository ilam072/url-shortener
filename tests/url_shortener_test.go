...
func TestURLShortener_SaveRedirect(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			respDel := e.DELETE("/url").
				WithJSON(del.Request{
					Alias: alias,
				}).
				WithBasicAuth("myuser", "mypass").
				Expect().
			...
			respDel.Value("alias").String().IsEqual(alias)

			testReditectNotFound(t, alias)

			// TODO: add remove (done)
		})
	}
}
...
func testReditectNotFound(t *testing.T, alias string) {
	u := url.URL{
		Scheme: "http",
		Host:   host,
		Path:   alias,
